       ...
       <script>
           ...
           demo.step = function(time, lag) {
               ...
               for (var i = rain.length - 1; i >= 0; i--) {
                   ...
                   // recycle rain
                   if (r.y > height + Rain.height * r.z || (wind < 0 && r.x < wind)
           ...
           function Drop() {
               ...
               var grd = this.ctx.createRadialGradient(this.radius, this.radius, 1,
           ...
           var Ticker = {
               _started: false,
               _last_timestamp: 0,
               _frameHandler: function(timestamp) {
                   var Ticker = window.Ticker;
                   var frame_time = timestamp - Ticker._last_timestamp;
                   Ticker._last_timestamp = timestamp;
                   ...
                   for (var i = 0, len = Ticker._listeners.length; i < len; i++) {
                       Ticker._listeners[i].call(window, frame_time, frame_time / 16.67);
                   }
                   // queue up next frame
                   if (window.requestAnimationFrame) {
                       requestAnimationFrame(Ticker._frameHandler);
                   } else {
                       webkitRequestAnimationFrame(Ticker._frameHandler);
                   }
               },
               _listeners: [],
               ...
               addListener: function(fn) {
                   ...
                   if (!this._started) {
                       ...
                       if (window.requestAnimationFrame) {
                           ...
                           webkitRequestAnimationFrame(this._frameHandler);
                       }
                   }
               }
           };
